...
App({
  onLaunch: function () {
    var logId = wx.getStorageSync('logId') || false;
    var that = this;
    var term = wx.getStorageSync('TERM') || false;
    if(!term) wx.setStorageSync('TERM', this.getTerm());
    if (logId == this.getVersion()) return;
    wx.request({
      url: that.getApi('LOG_VERSION') + that.getVersion(),
      success: function (res) {
        ...
        wx.setStorageSync('logId', that.getVersion());
        wx.showModal({
          title: '更新日志 ' + that.getVersion(),
          content: content,
          showCancel: false
        })
      }
